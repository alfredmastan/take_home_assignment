...
- [ ] Add `anthropic>=0.52` to `pyproject.toml` and run `uv sync`
- [ ] Create `reznar/parse_pdf.py`
  - Render each of the 39 PDF pages to a base64 PNG via `pypdfium2` (scale=2)
  - Send each page image to `claude-haiku-4-5-20251001` vision
...
- [ ] Create `reznar/extract.py` — reads `data/items_raw.json`, analyzes each item description one at a time with a text LLM, populates Postgres via `db.py`
