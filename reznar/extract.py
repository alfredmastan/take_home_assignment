...
_SYSTEM = (
    ...
    "CHARGES & USAGE LIMITS: How often an item can be used is a LIMITATION, never a special_effect. "
    "Set charges to the count of uses (small integer): 'three times before a long rest' → 3, 'once per "
    "short rest' → 1, 'fifty times before exhausted' → 50. Recharge/cooldown timing ('a long rest must "
    "pass', '240 hours to recharge') goes in drawbacks as a short phrase. Capacity measurements — "
    "pounds of material, cubic feet, distance, weight — are NOT charges; describe those in drawbacks "
    "and leave charges null. Never restate any usage limit as a special_effects entry.\n\n"
    "SPECIAL EFFECTS & DRAWBACKS (granularity): special_effects and limitations.drawbacks are lists "
    "of DISTINCT effects/penalties — their length is used downstream as a proxy for how many different "
    "things an item does, so granularity must be consistent.\n"
    "- ONE entry per genuinely DISTINCT effect — a distinct thing the item does or change it makes "
    "(deal bonus damage, control a creature, grant a sense, impose a condition, conjure gear, "
    "teleport, turn invisible, etc.).\n"
    "- FOLD an effect's own parameters into its single entry: range/area, save DC, duration, and "
    "magnitude scaling. A resist save that scales +0/+2/+3 by enemy tier or dragon age stays INSIDE "
    "that one effect — it merely tunes it. Example: 'Horn of Bronze Dragon Control' dominating one "
    "dragon for 1 hour (save scaling with age, re-save on damage) is ONE entry; being able to force "
    "the dominated dragon to use reactions is a second, distinct effect → two entries total.\n"
    "- SPLIT genuinely different effects into separate entries — INCLUDING a selector that switches "
    "between different MECHANICS. 'Lunar Potion' grants a different effect per moon phase (a stat "
    "bonus, OR invisibility, OR necrotic-on-hit, OR auto-stabilize, ...) → ONE entry per phase. By "
    "contrast a selector that only tunes the flavor/magnitude of ONE mechanic stays ONE entry: "
    "'Amulet of Amplified Emotion' amplifies one chosen emotion (gemstone color just picks which) → "
    "ONE entry. Litmus: different mechanics → split; same mechanic, different magnitude/flavor → fold.\n"
    "- No duplication: never list something already captured by a structured field. Do not restate "
    "AC or attack bonuses, damage resistances, condition immunities, creature resistances, charges, "
    "usage limits, the cursed flag, slot, form, or attunement.\n"
    "- Drawbacks follow the same per-distinct-penalty rule and must be grounded: only record a "
    "drawback explicitly stated in the description; never infer or invent one. Most items have no "
    "drawback — an empty list (or no limitations component) is the correct, expected output. Do not "
    "pad it to make an item look costlier."
)
